...
class Backup {
    ...
    async init() {
        ...
        for await (const item of Deno.readDir(opts.dir)){
            ...
            if (found) {
                throw new Error(`target id(${id}) already exists: [${found.name}, ${name}]`);
            }
    ...
    async create() {
        ...
        if (targets.length == 0) {
            ...
            if (target.completed) {
                const id1 = target.id + 1;
                const name1 = `target.${id1}.${dateNow()}`;
                target = new Target(id1, name1, joinPath(opts.dir, name1));
                await Deno.mkdir(target.path, {
                    mode: 0o775
                });
                targets.push(target);
...
class Target {
    ...
    async init() {
        ...
        for await (const item of Deno.readDir(path)){
            ...
            const checkpoints = await Checkpoints.load(joinPath(p, "xtrabackup_checkpoints"));
            const found = keys.get(id);
            if (found) {
                throw new Error(`${path} history id(${id}) already exists: ${found.name} ${name}`);
            }
            const node = {
                id: id,
                name: name,
...
Cron.Cron = Cron;
async function writeServerID(id) {
    log.info(`server-id=${id}`);
    await Deno.writeTextFile(`/etc/mysql/conf.d/server-id.cnf`, `[mysqld]
server-id=${id}
`, {
        mode: 0o664
    });
...
class Service {
    ...
    constructor(){
        ...
        if (rootPassword == "") {
            throw new Error(`unknow root password, please set environment variable 'MYSQL_ROOT_PASSWORD' `);
        }
